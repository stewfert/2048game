       ...
       Font blockFont= new Font( "SansSerif", Font.PLAIN, 10 );
        for (int j=0; j<blockArray.length; j++){
            for (int i=0; i<blockArray.length; i++){
                ...
                int blockTextWidth=g2.getFontMetrics().stringWidth(Integer.toString(blockArray[i][j].getValue()));     
               
                int textHeight= 10;
                if(blockArray[i][j].isBlock()){
                    Color blockColor=new Color(blockArray[i][j].getRed(),blockArray[i][j].getGreen(),blockArray[i][j].getBlue());
                g2.setColor(blockColor);
                g2.fillRect(blockXPos,blockYPos, BLOCK_SIZE,BLOCK_SIZE);
                g2.setFont(blockFont);
                g2.setColor(Color.BLACK);
                g2.drawString(Integer.toString(blockArray[i][j].getValue()),blockXPos-(blockTextWidth/2)+(BLOCK_SIZE/2),blockYPos+(BLOCK_SIZE/2)+(textHeight/2));
        ...
        String printScore=("Your score is "+score.getScore());
         int scoreWidth=g2.getFontMetrics().stringWidth(printScore); 
         System.out.println(scoreWidth);
        g2.drawString(printScore,(PANEL_X_SIZE/2)-(scoreWidth/2),75);
    } 
