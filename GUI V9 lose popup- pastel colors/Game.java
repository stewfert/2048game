...
public class Game extends JPanel implements KeyListener {
    ...
    public void keyPressed(KeyEvent e) {
        ...
        switch(input){
            ...
            case 38: 
                runTurn(false, false);
                //up arrow key pressed (up movement)
                break;
            case 39: 
                runTurn(true, true);
                //right arrow key pressed (right movement)
                break;
            case 37: 
                runTurn(true, false);
                //left arrow key pressed (left movement)
                break;
            case 40: 
                runTurn(false, true);
                //down arrow key pressed (down movement)
                break;
            default: 
    ...
    public void runTurn(boolean xPressed, boolean posDirection){
        ...
        //so as to not spawn blocks without merges or moves we check if anything has changed
        
        if( activityCheck==true){
            spawnBlock();
            newMove=false;
        }
        //if a key input doesn't result in a move the game informs the user through a beep.
        else if(newMove==false){
            playSound("invalid.wav");
            //sound from freesound under creative commons https://freesound.org/people/Entershift/sounds/704134/
            //stops the program from beeping if new moves are made
            newMove=true;
        }
        if (loseCheck()==false){
            repaint();
    ...
    //method that adds two blocks next to each other in an inputted direction.
    public void mergeBlocks(boolean Xpressed, boolean posDirection){
        int parseFromX=0, parseFromY=0;
        int dX=0, dY=0;
        int xCheck=0, yCheck=0;
        boolean gameWon=false; //if the player has reached 2048
        ...
        //for loop that parses through the array checking if merges can be made
            ...
            for (int i=parseFromX; i>=0 && i<blockArray.length; i+=dX){
                try{
                    ...
                    && blockArray[i][j].canMerge()==true) {
                        ...
                        //if the player reaches the 2048 tile
                        if(blockArray[i+xCheck][j+yCheck].getValue()==2048){
        ...
        //so that when the player reaches 2048 the game doesn't freeze pre merge
        if(gameWon==true){
            repaint();
            Main.gameWon();
        }
    ...
    //removes the merge property of blocks that have been merged.
    ...
    //to move the blocks in the direction inputted by the user
    ...
    //creates a new space in a given space
    public void newSpace(int x, int y){
        blockArray[x][y]=new newBlock("space", 0);
    }

    //method for testing my graphics by printing out the contents of the array, not used anymore.
    ...
    //method that tests if the merge calculations have functioned correctly, parses the array and highlights blocks that have been marked for merges. Not used anymore
    ...
    public void spawnBlock(){
        ...
        //if a position a block tries to spawn is taken up the block is spawned in a new location.
        do{
    ...
    //method that plays wav tracks from file. Method written by Joel & Harrison.
    public void playSound(String fileName) {
        if (!soundPlaying) {
            try { 
                sound = AudioSystem.getClip();
                //Opens and prepares the audio file from the folder audio. 
                sound.open(AudioSystem.getAudioInputStream(getClass().getResource("audio/" + fileName)));
                soundPlaying = true;
            } catch (Exception e) {
                e.printStackTrace();
                System.out.println("Error occurred whilst attempting to play audio from file");
            }
        }
        if (soundPlaying) {
            sound.start();
    ...
    public void paint(Graphics g){
        //graphics object drawn to every turn. 
        super.paint(g);
        Graphics2D g1 = (Graphics2D) g;
        int spacing=60;
        int blockXPos=0;
        int blockYPos=0;
        //font for the blocks, bold and big so its easily readible.
        Font blockFont= new Font( "SansSerif", Font.BOLD,14);

        int PANEL_X_SIZE=400, PANEL_Y_SIZE=400;
        //colors for specific cases that are similar to 2048's.
        Color spaceColor= new Color( 246, 230, 222);
        Color scoreColor= new Color(82,73,76);
        for (int j=0; j<blockArray.length; j++){
            for (int i=0; i<blockArray.length; i++){
                ...
                if(blockArray[i][j].isBlock()){
                    ...
                    //centers the value of the block, don't have to worry about text not properly displaying as max possible block fits in the block size.
        ...
        //so that the centering can be calculated with the font size
        ...
        g1.drawString(printScore,(PANEL_X_SIZE/2)-(scoreWidth/2),70);
