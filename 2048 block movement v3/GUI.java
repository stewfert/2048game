    ...
    public void moveBlocks(boolean Xpressed,boolean posDirection){
        ...
        for (int j=parseFromY; j>=0 && j<blockArray.length; j+=dY){
            for (int i=parseFromX; i>=0 && i<blockArray.length; i+=dX){
                ...
                while(checkSpaceEmpty(moveX,moveY) && (moveX+=dX)>0 && (moveX+=dX)<4){
                moveX+=dX;
                System.out.println("bruhhhhh");
                }
                System.out.println(moveX+" is the X value the block can move to "+i);
            }
            else if(blockArray[i][j].isBlock()){
                while(checkSpaceEmpty(moveX,moveY) && (moveY+=dY)>0 && (moveY+=dY)<4){
                moveY+=dY;
                }
                 System.out.println(moveY+" is the Y value the block can move to from "+j);
