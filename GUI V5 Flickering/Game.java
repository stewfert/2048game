 ...
 */ 
 ...
 //method for character keys typed, don't need this function for my game .
    ...
    //method that's run at the start of the game to setup the board
    public void gameMethod(){
       

        
        //Create a new Jpanel to hold my Jframe and graphics
        JPanel panel = new JPanel();

        panel.setPreferredSize(new Dimension(PANEL_X_SIZE,PANEL_Y_SIZE));
        Toolkit toolkit= getToolkit();
        Dimension size = toolkit.getScreenSize();
        //center the new JPanel on the screen
        setLocation((size.width/2)-(PANEL_X_SIZE/2),(size.height/2)-(PANEL_Y_SIZE/2));
    ...
    //loops through every element in the array and paints a square on the screen to represent it.   
        for (int j=0; j<blockArray.length; j++){
            for (int i=0; i<blockArray.length; i++){
                //variable for determining where the blocks should be drawn in relation to other blocks.
                blockXPos=(PANEL_X_SIZE/2)-30-(BLOCK_SIZE/2) +(i*spacing);
                blockYPos=(PANEL_Y_SIZE/2)-30-(BLOCK_SIZE/2) +(j*spacing);
                //text height and width to easier center text. textHeight cannot be used with font metrics as height returns interesting numbers.
                int blockTextWidth=g1.getFontMetrics().stringWidth(Integer.toString(blockArray[i][j].getValue()));     
                int textHeight= 10;
                if(blockArray[i][j].isBlock()){
                    //these lines draw the boxes for the blocks in the game and put their value in the center of the blocks.
                ...
                //these lines add the blocks value to the center of the blocks.
