    ...
    public void moveBlocks(boolean Xpressed,boolean posDirection){
        ...
        for (int j=parseFromY; j>=0 && j<blockArray.length; j+=dY){
            for (int i=parseFromX; i>=0 && i<blockArray.length; i+=dX){
                ...
                int moveX=i, moveY=j;
                if(Xpressed==true && blockArray[moveX][moveY].isBlock() && (i+=dX)>0 && (i+=dX)<4){
                    moveX+=dX;
                    do{
                        try{
                            if(!blockArray[moveX][moveY].isBlock() && moveX>=0 && moveX<blockArray.length){
                                ...
                                int storeBlockValue=blockArray[i][j].getValue();
                                newSpace(i,j);
                                blockArray[moveX][moveY]=new newBlock("block", storeBlockValue);
                ...
                System.out.println("huh?");
                    ...
                    if(Xpressed==false && blockArray[moveX][moveY].isBlock()){
                        moveY+=dY;
                        do{
                            try{
                                if(!blockArray[moveX][moveY].isBlock() && moveY>=0 && moveY<blockArray.length){
                                    moveY+=dY;
                                }
                                else{
                                    int storeBlockValue=blockArray[i][j].getValue();
...
System.out.println("whatsa going on here");
                                runMove=false;}
